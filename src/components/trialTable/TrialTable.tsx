...
export default function TrialTable() {
  ...
  const getPlatformIcon = (url: string) => {
    if (url.includes("twitter")) return <Twitter className="text-blue-400" size={20} />;
    if (url.includes("youtube")) return <Youtube className="text-red-500" size={20} />;
    if (url.includes("chrome")) return <Chrome className="text-blue-500" size={20} />;
  ...
  return (
    <div className="overflow-x-auto">
      ...
      <div className="text-sm text-gray-400 mt-4 text-center">
        You can create{" "}
        <span className="text-pink-500">{remainingTrials > 0 ? remainingTrials : 0}</span> more
        links.{" "}
      ...
      <table className="w-full">
        ...
        <tbody>
          {trialUrls.length === 0 ? (
            ...
            trialUrls.map((item) => (
              <tr key={item.id} className="bg-[#181E29] border-t border-gray-800">
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <Link
                      ...
                      target="_blank"
                    >
                      {`http://localhost:3000/${item.shortCode}`} {/* Changed to localhost */}
                    </Link>
                    <button
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="truncate max-w-xs">{item.originalUrl}</span>
                  </div>
                </td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
